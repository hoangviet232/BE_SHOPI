...
try {
            ...
            if (!isEmage || isNullOrEmpty) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ResponseDTO(404, "??NH D?NG FILE KH�NG ?�NG HO?C R?NG!", null));
            }
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseDTO(404, "KH�NG T�M TH?Y LO?I S?N PH?M! ", productResult));
        }
    ...
    public ResponseEntity<?> deleteProduct(long id) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseDTO(404, "XO�? SẢN PHẨM KHÔNG THÀNH CÔNG!", new ArrayList<>()));
        }

        return ResponseEntity.status(HttpStatus.OK).body(new ResponseDTO(200, "XO�? SẢN PHẨM THÀNH CÔNG!", new ArrayList<>()));
    }
